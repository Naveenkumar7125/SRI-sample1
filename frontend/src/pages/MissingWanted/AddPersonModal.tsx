...
//     // Missing-specific
//     if (status === "Missing") {
//       formData.append("lastSeenDateTime", lastSeenDate);
//       formData.append("lastSeenLocation", location);
//     }

//     // Wanted-specific
//     if (status === "Wanted") {
//       formData.append("crimeOrReason", crime);
//       formData.append("reward", reward === "" ? "" : String(reward));
//     }

//     // Attach images (actual files)
//     images.forEach((img) => {
...
//       "http://localhost:5000/api/criminals",
//       formData,
//       { headers: { "Content-Type": "multipart/form-data" } }
//     );

//     // Backend returns saved record with Cloudinary URLs
//     const savedRecord = res.data.data;

//     // Send the saved record to parent so UI updates
//     onAdd({
//       id: savedRecord._id,
//       name: savedRecord.fullName,
//       age: savedRecord.age,
//       status: savedRecord.status,
//       location: savedRecord.lastSeenLocation,
//       lastSeenDate: savedRecord.lastSeenDateTime,
//       crime: savedRecord.crimeOrReason,
//       reward: savedRecord.reward,
//       contact: savedRecord.contact,
//       description: savedRecord.description,
//       images: savedRecord.images.map((url) => ({
//         id: uid("img-"),
//         url
//       }))
//     });
...
import { X, PlusCircle, Camera, Trash2 } from "lucide-react";
import axios from "axios";
import { Status, PersonRecord, ImageItem } from "./types";
...
export default function AddPersonModal({ open, onClose, onAdd, uid }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // POST request to backend (ensure backend URL & CORS)
      const res = await axios.post("http://localhost:5000/api/criminals", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      const savedRecord = res.data?.data ?? null;

      // fallback: if backend not available, create a client-side record
      if (!savedRecord) {
        const fallback: PersonRecord = {
          id: uid("rec-"),
          name,
          age,
          status,
          ...
          contact,
          description,
          images: images.map((it) => ({ id: it.id, url: it.preview })),
        };
        onAdd(fallback);
      } else {
        onAdd({
          ...
          contact: savedRecord.contact,
          description: savedRecord.description,
          images: (savedRecord.images || []).map((url: string) => ({ id: uid("img-"), url })),
        });
      }
          ...
          <div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 mt-3">
              {images.length === 0 ? (
                ...
                images.map((img) => (
                  <div key={img.id} className="relative bg-card-bg rounded-md border border-sidebar-border overflow-hidden">
                    <img src={img.preview || img.url} className="h-24 w-full object-cover" alt="" />
                    <button type="button" className="absolute top-1 right-1 p-1 bg-black/50 rounded-full" onClick={() => removeImage(img.id)}>
                      <Trash2 className="h-4 w-4 text-white" />
                    </button>
                  </div>
